import { html, state, web_component, define_component } from "https://busy-dog-44.deno.dev/melhosseiny/sourdough/main/sourdough.js";

const template = (data) => html`
  <dialog>
    <nav id="social-links">
      <a class="button" title="Discourse" href="https://meta.discourse.org/u/melhosseiny" target="_blank" rel="noreferrer">
        <svg class="symbol" xmlns="http://www.w3.org/2000/svg" viewBox="0 -1 104 106">
          <path fill="#231f20" d="M51.87 0C23.71 0 0 22.83 0 51v52.81l51.86-.05c28.16 0 51-23.71 51-51.87S80 0 51.87 0Z"/>
          <path fill="#fff9ae" d="M52.37 19.74a31.62 31.62 0 0 0-27.79 46.67l-5.72 18.4 20.54-4.64a31.61 31.61 0 1 0 13-60.43Z"/>
          <path fill="#00aeef" d="M77.45 32.12a31.6 31.6 0 0 1-38.05 48l-20.54 4.7 20.91-2.47a31.6 31.6 0 0 0 37.68-50.23Z"/>
          <path fill="#00a94f" d="M71.63 26.29A31.6 31.6 0 0 1 38.8 78l-19.94 6.82 20.54-4.65a31.6 31.6 0 0 0 32.23-53.88Z"/>
          <path fill="#f15d22" d="M26.47 67.11a31.61 31.61 0 0 1 51-35 31.61 31.61 0 0 0-52.89 34.3l-5.72 18.4Z"/>
          <path fill="#e31b23" d="M24.58 66.41a31.61 31.61 0 0 1 47.05-40.12 31.61 31.61 0 0 0-49 39.63l-3.76 18.9Z">
        </svg>
        Discourse Meta
      </a>
      <a class="button" title="Glitch" href="https://glitch.com/@melhosseiny" target="_blank" rel="noreferrer">
        <svg class="symbol" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 614.93 478.48">
          <path d="M243,242.33V224.71H461.72v3.42c-28,9.12-69.46,17.72-80.1,19.38,5.8,2.7,7.69,5.08,10.16,5.89a208.75,208.75,0,0,1,25.42,4.68c16.47,3.79,30.11,7.38,41.33,11.34,11.66,4.36,17.83,6.37,24.7,8.35a246,246,0,0,0,24.36,5.33c6.77,1,21.16-4,24.54-4.87s47.15-17.88,47.15-17.88,19-7.11,26.52-9c6.91-1.91,7.09.4,7.09.4a47.43,47.43,0,0,1,1.48,6.11,100.59,100.59,0,0,1,.18,19.59,149.43,149.43,0,0,1-3,14.93A65.07,65.07,0,0,1,607,304.16c-1.4,2.45-3.56,6.27-4.3,7.93a22,22,0,0,0-2.18,7.29,35.09,35.09,0,0,0,.49,10.29c.6,2.9,2.54,10.17,2.54,10.17l2.29,9.09-313.46-.38,0,18.38H38.53a2.51,2.51,0,0,0,.07-1.31c-.26-1.12-1.54-2.77-6.44-5.72-6.31-4.11-26.23-11.32-32-13.43C-.07,341-2.44,315.8,29,288.15a297.4,297.4,0,0,1,29.66-22.3s27-17.66,48.46-24.71" fill="#2800ff" style="isolation:isolate"/>
          <path d="M605.79,348.93l-313.46-.38,0,18.38H38.53c-1.41,4.14-20.46,11.46-33.5,14,7,9.63,20.22,18.42,33.49,26a304.23,304.23,0,0,0,39.72,18.11c5,1.73,19.9,7.56,28.78,8.26a206.19,206.19,0,0,0,37.08-.7,173.16,173.16,0,0,0,19.37-3s1.24,4.9,9,15.66c5.61,7.45,12.49,15.07,19.36,19.58,8.18,5.17,10.9,7.9,28.62,11.13a141.64,141.64,0,0,0,40,1.86c9-.83,17.72-1.12,25.47-3.27.37-9.94-.88-19.12-4.88-27.23,10.76.43,16.74.74,31.76.45,29-.55,60.67-6.52,86.63-11.48,21.45-4.09,56.54-13.92,73.32-19.5,9.64-3.21,32.56-11,34.69-11,3.45,0,8.63.81,15.84,2.94,10.71,3.17,18.94,6,37,12.49s34.13,12.15,43.91,13.49c7.23,1,8.12,0,8.62-3.62.83-4.58-5.55-15-8.18-23.18a43.14,43.14,0,0,1-1.89-26,70.16,70.16,0,0,1,2.46-7.5,41,41,0,0,0,2.24-15.55A48,48,0,0,0,605.79,348.93Z" fill="#5a78ff"/>
          <path d="M601.7,198.75a33,33,0,0,1,1.3-10.19c.62-2,1.26-3.78,2.44-7.15a40.44,40.44,0,0,0,1.89-15.93,82,82,0,0,0-1.57-9.49l-229.88-.39v17.9H38.68c.68,4.91-20.5,12.76-33.54,15.34C11.94,198.18,25.69,207.39,39,215a311.06,311.06,0,0,0,39.78,18.11c6,2.12,19.12,7,28.4,8.07L243,242.33V224.71H461.72v3.42c3.3-.92,8.2-2.38,11.35-3.42,9.64-3.21,32.94-11.23,35.07-11.26,3.46,0,7.87.76,15.08,2.89,10.71,3.16,19.23,6.33,37.31,12.78s33.77,12,43.53,13.44c8.39,1.25,8.89-1.56,8.89-4.91.2-4.27-6.12-15.06-8.76-23.28A50.64,50.64,0,0,1,601.7,198.75Z" fill="#fec1ee"/>
          <path d="M614.37,65.91c.18,3.26.68,16.56.18,19.58a113.94,113.94,0,0,1-2.8,13.69c-1.3,4.1-3.28,10-4.8,12.68-1.4,2.45-3.68,6.23-4.42,7.89a24.77,24.77,0,0,0-2.06,7.69,35.09,35.09,0,0,0,.49,10.29c.6,2.9,2.54,10.17,2.54,10.17l2.26,8.09-229.88-.39v17.9H38.68s.3-2-5.8-5.8c-5.5-3-23.4-10.6-32.5-13.4C-.5,149.73-2.44,123.85,29,96.21A295.82,295.82,0,0,1,58.68,73.9s26.08-18.51,54.1-26.8a142.37,142.37,0,0,1,20.6-4.2c19.87-1.62,51.72-2.4,54.84-2.29A231.72,231.72,0,0,1,208.58,17C220.45,5.31,227.18.8,234.28,0c9.8.58,62.26,12.84,77.07,17.25,8.5,2.4,25.52,7,39,12.65,13.9,5.34,17.28,9.88,17.28,9.88a124.57,124.57,0,0,0,8.46,11.33c3.28,3.28,12.7,8.94,15.32,10,3.95,1,10.63,1.06,25.75,4.71,15.8,3.76,30,7.78,41.3,11.3,11.7,3.9,17.8,6.6,24.8,8.4s18.2,4.5,24.4,5.4,19.83-4,23.21-4.87S579.28,68.4,579.28,68.4s24.3-8.69,26.5-9.4C613.15,56.6,614.37,56.43,614.37,65.91Z" fill="#ff7698" style="isolation:isolate"/>
          <path d="M107,93.51c-15.9,0-27.9,12.2-28.7,29.5-.8,17.1,12.9,29.5,28.7,29.5s28.7-13.2,28.7-29.5S122.91,93.51,107,93.51Zm-6.7,52.8c-12.2,0-22.1-8.7-22-22.7.4-11.5,9.9-22.7,22-22.7s22,10.1,22,22.7S112.51,146.31,100.31,146.31Z" fill="#fff"/>
          <path d="M100.31,101c-12.2,0-21.7,11.2-22,22.7,0,14,9.9,22.7,22,22.7s22-10.1,22-22.7S112.51,101,100.31,101Zm-10.4,37.8a7.4,7.4,0,1,1,7.2-7.4A7.28,7.28,0,0,1,89.91,138.81Z"/><ellipse cx="89.91" cy="131.41" rx="7.2" ry="7.4" fill="#fff"/>
          <path d="M107,287.09c-15.9,0-27.9,12.2-28.7,29.5-.8,17.1,12.9,29.5,28.7,29.5s28.7-13.2,28.7-29.5S122.91,287.09,107,287.09Zm-6.7,52.8c-12.2,0-22.1-8.7-22-22.7.4-11.5,9.9-22.7,22-22.7s22,10.1,22,22.7S112.51,339.89,100.31,339.89Z" fill="#fff"/>
          <path d="M100.31,294.59c-12.2,0-21.7,11.2-22,22.7,0,14,9.9,22.7,22,22.7s22-10.1,22-22.7S112.51,294.59,100.31,294.59Zm-10.4,37.8a7.4,7.4,0,1,1,7.2-7.4A7.28,7.28,0,0,1,89.91,332.39Z"/><ellipse cx="89.91" cy="324.99" rx="7.2" ry="7.4" fill="#fff"/>
          <path d="M224.46,173.5c-4.11,15.43-8.38,28.13-8.46,28.36a7,7,0,0,0,13.27,4.49c.34-1,5.26-15.65,9.68-32.85Z" fill="#5a78ff"/>
          <path d="M192.24,173.5c-1.28,2.89-2.2,4.66-2.25,4.76a6,6,0,0,0,10.6,5.61,113.2,113.2,0,0,0,4.68-10.37Z" fill="#5a78ff"/>
          <path d="M231.9,135.15c0,.21-.06.41-.08.62-.8,10.93-4,25.23-7.36,37.73H239c3.12-12.14,6-25.57,6.81-36.42,1.78-12.09-5.61-34.81-10.5-48.08a1196.75,1196.75,0,0,1,125.12,11.37c71.07,10.28,156.8,52.88,157.66,53.31a7,7,0,1,0,6.26-12.52c-3.58-1.8-88.58-44-161.91-54.65-76-11-136.79-11.78-137.39-11.78a7,7,0,0,0-6.55,9.67C224.48,99,233.46,125.62,231.9,135.15Z" fill="#2800ff"/>
          <path d="M200.57,136.58c1.17,13.32-4.88,29.16-8.33,36.92h13c3.84-9.64,8.44-24.47,7.25-38a111.21,111.21,0,0,0-17.8-50.8,6,6,0,0,0-9.76,7A100.88,100.88,0,0,1,200.57,136.58Z" fill="#2800ff"/>
          <path d="M224.59,366.89c-4.11,15.43-8.38,28.13-8.46,28.36a7,7,0,0,0,13.26,4.49c.35-1,5.27-15.65,9.68-32.85Z" fill="#fec1ee"/>
          <path d="M192.37,366.89c-1.29,2.89-2.21,4.66-2.26,4.76a6,6,0,0,0,10.61,5.61,113.2,113.2,0,0,0,4.68-10.37Z" fill="#fec1ee"/>
          <path d="M232,328.54a6.18,6.18,0,0,0-.07.62c-.81,10.93-4,25.23-7.36,37.73h14.48c3.12-12.14,6-25.57,6.82-36.42,1.78-12.09-5.61-34.81-10.51-48.08a1196.87,1196.87,0,0,1,125.13,11.37c71.06,10.28,156.8,52.88,157.65,53.31a7,7,0,1,0,6.27-12.52c-3.58-1.8-88.58-44-161.92-54.65a1167.85,1167.85,0,0,0-137.39-11.79,7,7,0,0,0-6.54,9.68C224.61,292.35,233.59,319,232,328.54Z" fill="#ff7698"/>
          <path d="M200.69,330c1.18,13.32-4.88,29.16-8.32,36.92h13c3.83-9.64,8.44-24.47,7.25-38a111.21,111.21,0,0,0-17.8-50.8,6,6,0,0,0-9.76,7A100.94,100.94,0,0,1,200.69,330Z" fill="#ff7698"/>
        </svg>
        Glitch
      </a>
      <a class="button" title="Stack Overflow" href="https://stackoverflow.com/users/371309/melhosseiny" target="_blank" rel="noreferrer">
        <svg class="symbol" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 32 37">
          <path d="M26 33v-9h4v13H0V24h4v9h22Z" fill="#BCBBBB"></path>
          <path d="m21.5 0-2.7 2 9.9 13.3 2.7-2L21.5 0ZM26 18.4 13.3 7.8l2.1-2.5 12.7 10.6-2.1 2.5ZM9.1 15.2l15 7 1.4-3-15-7-1.4 3Zm14 10.79.68-2.95-16.1-3.35L7 23l16.1 2.99ZM23 30H7v-3h16v3Z" fill="#F48024"></path>
        </svg>
        Stack Overflow
      </a>
      <a class="button" title="Nifty Gateway" href="https://www.niftygateway.com/@kwisatzhaderach" target="_blank" rel="noreferrer">
        <svg style="background-image: linear-gradient(94.8deg, rgb(16, 227, 194) 0%, rgb(12, 184, 210) 100%); border-radius: 3px; padding-top: 0; margin-top: 2px;" class="symbol" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 228 233" fill="none">
          <path d="M140.053 60.876V142.789L87.3825 60.876H64.1211V177.183H85.2225V94.2727L138.89 177.183H161.154V60.876H140.053Z" fill="currentColor"></path>
        </svg>
        Nifty Gateway
      </a>
      <a class="button" title="LeetCode" href="https://leetcode.com/melhosseiny" target="_blank" rel="noreferrer">
        <svg class="symbol" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 95 111" fill="none">
          <path d="M68.0063 83.0664C70.5 80.5764 74.5366 80.5829 77.0223 83.0809C79.508 85.579 79.5015 89.6226 77.0078 92.1127L65.9346 103.17C55.7187 113.371 39.06 113.519 28.6718 103.513C28.6117 103.456 23.9861 98.9201 8.72653 83.957C-1.42528 74.0029 -2.43665 58.0749 7.11648 47.8464L24.9282 28.7745C34.4095 18.6219 51.887 17.5122 62.7275 26.2789L78.9048 39.362C81.6444 41.5776 82.0723 45.5985 79.8606 48.3429C77.6488 51.0873 73.635 51.5159 70.8954 49.3003L54.7182 36.2173C49.0488 31.6325 39.1314 32.2622 34.2394 37.5006L16.4274 56.5727C11.7767 61.5522 12.2861 69.574 17.6456 74.8292C28.851 85.8169 37.4869 94.2846 37.4969 94.2942C42.8977 99.496 51.6304 99.4184 56.9331 94.1234L68.0063 83.0664Z" fill="#FFA116"></path>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M41.1067 72.0014C37.5858 72.0014 34.7314 69.1421 34.7314 65.615C34.7314 62.0879 37.5858 59.2286 41.1067 59.2286H88.1245C91.6454 59.2286 94.4997 62.0879 94.4997 65.615C94.4997 69.1421 91.6454 72.0014 88.1245 72.0014H41.1067Z" fill="#B3B3B3"></path>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M49.9118 2.02335C52.3173 -0.55232 56.3517 -0.686894 58.9228 1.72277C61.494 4.13244 61.6284 8.17385 59.2229 10.7495L16.4276 56.5729C11.7768 61.552 12.2861 69.5738 17.6453 74.8292L37.4088 94.2091C39.9249 96.6764 39.968 100.72 37.505 103.24C35.042 105.761 31.0056 105.804 28.4895 103.337L8.72593 83.9567C-1.42529 74.0021 -2.43665 58.0741 7.1169 47.8463L49.9118 2.02335Z" fill="black"></path>
        </svg>
        LeetCode
      </a>
      <a class="button" title="Open Powerlifting" href="https://www.openpowerlifting.org/u/mostafaelshamy" target="_blank" rel="noreferrer">
        <svg class="symbol" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1982 652">
          <g fill="none" fill-rule="nonzero">
            <g fill="#000">
              <path d="M361 449.28h72.92c6.57 0 11.45 1.21 14.55 3.78 3.08 2.54 4.58 6.71 4.58 12.66v18.9c0 5.94-1.48 10.12-4.6 12.67-3.21 2.56-7.97 3.78-14.55 3.78h-53.6v33.16H361v-84.95zm19.54 14.69v22.63h44.48c3.07 0 5.38-.28 6.71-1.12 1.5-.84 2.17-2.66 2.17-5.46v-8.96c0-2.8-.7-4.61-2.24-5.45-1.54-.7-3.64-1.12-6.71-1.12h-44.51l.1-.52zM478.7 469.37c0-3.92.42-7.28 1.08-9.85.67-2.7 1.89-4.72 3.5-6.2a13.34 13.34 0 0 1 6.47-3.1c2.65-.43 6.01-.84 9.93-.84h62.56c3.92 0 7.28.27 9.94.97 2.66.56 4.76 1.68 6.44 3.22 1.54 1.54 2.8 3.5 3.5 6.3.7 2.66 1.12 5.99 1.12 9.9v44.87c0 3.92-.42 7.28-1.12 9.85a12.95 12.95 0 0 1-3.5 6.16 13.24 13.24 0 0 1-6.58 3.08c-2.8.56-6.16.98-10.08.98h-62.4c-3.92 0-7.28-.28-9.98-.98a13.54 13.54 0 0 1-6.5-3.08c-1.6-1.54-2.8-3.5-3.5-6.21a40.02 40.02 0 0 1-1.07-9.8l.2-45.27zm85.17 49.03v-53.9H498.5v53.9h65.36zM603.86 449.28h20.48l21.55 57 22.68-57h13.85l23.05 57 20.62-57h17.66l-32.89 85.02h-13.34l-23.45-58.89-23.8 58.9h-13.71zM765.81 449.28h87.05v14.55h-67.23v19h39.21v14.55h-39.21v21.13h68.06v16.05h-87.9zM878.14 449.28h72.91c6.58 0 11.45 1.21 14.56 3.78 3.08 2.54 4.57 6.71 4.57 12.66v19.87c0 5.88-1.48 10.07-4.59 12.6-3.21 2.51-7.97 3.77-14.55 3.77h-11.2l39.19 32.19h-28.27l-34.43-32.19h-18.9v32.19h-19.3v-84.87zm19.53 14.69v23.7h44.48c3.08 0 5.39-.42 6.72-1.2 1.5-.81 2.17-2.7 2.17-5.53V470.7c0-2.83-.7-4.71-2.24-5.53-1.54-.8-3.64-1.21-6.72-1.21l-44.4.01zM998.27 449.28h19.82v69.12h63.88v16.04h-83.56v-85.16zM1104.84 449.28h19.81v85.02h-19.81zM1146.85 449.28h86.07v14.83h-66.31v22.36h35.83v15.5h-35.83v32.31h-19.8zM1300.14 534.44h-19.81V464.5h-39.08v-15.22h98.11v15.22h-39.22zM1357.51 449.28h19.82v85.02h-19.82zM1399.23 449.28h13.48l59.85 50.12c1.74 1.48 3.35 2.83 4.98 4.31 1.6 1.48 3.08 3.1 4.71 4.86-.28-3.5-.42-6.44-.56-8.96-.1-2.38-.25-4.34-.25-5.6v-44.63h17.78v85.07h-13.47l-62.53-52.61c-1.2-1.12-2.42-2.1-3.5-3.22-1.22-.98-2.3-2.24-3.5-3.64.28 3.22.54 6.16.68 8.4.14 2.38.14 4.06.14 5.32v46.03h-17.8v-85.45zM1539.34 464.5v53.9h61.86v-19h-32.38v-14.55h51.61v29.38c0 3.9-.42 7.28-1.08 9.83a11 11 0 0 1-3.5 6.2 12.76 12.76 0 0 1-6.43 3.21c-2.66.56-6.02.98-9.94.98h-58.78c-3.92 0-7.28-.28-9.94-.84a13.21 13.21 0 0 1-6.44-3.07c-1.53-1.4-2.8-3.5-3.5-6.16-.7-2.66-.97-5.88-.97-9.8v-44.91c0-3.92.42-7.28 1.12-9.8.7-2.71 1.96-4.76 3.5-6.21a13.59 13.59 0 0 1 6.43-3.08 59.4 59.4 0 0 1 10.08-.88h58.64c3.92 0 7.14.28 9.94.84 2.7.56 4.85 1.62 6.44 3.08a13.24 13.24 0 0 1 3.63 5.96c.84 2.38 1.26 5.64 1.26 9.41v1.96l-19.31 3.5v-9.61l-62.24-.34z"/></g><g fill="#C1272D"><path d="M509.35 118.52c48.88 0 85.68 10.63 110.22 31.9 24.53 21.27 36.8 54.86 36.8 100.61 0 46.04-12.27 79.77-36.69 101.04-24.44 21.27-61.23 31.9-110.31 31.9-49.27 0-86.08-10.63-110.43-31.9-24.21-21.27-36.39-54.86-36.39-101.04 0-45.75 12.32-79.34 36.67-100.6 24.35-21.28 61.16-31.91 110.29-31.91h-.16zm0 198.56c15.68 0 28.83-.84 39.67-2.52 10.84-1.68 19.62-4.9 26.3-9.8 6.67-4.89 11.62-11.6 14.56-20.14 2.94-8.54 4.48-19.73 4.48-33.59 0-13.85-1.54-25.04-4.48-33.58-2.94-8.54-7.89-15.25-14.55-20.01-6.7-4.76-15.47-7.98-26.31-9.66-10.84-1.67-24.08-2.51-39.65-2.51-15.54 0-28.7.84-39.61 2.51-10.78 1.68-19.6 4.9-26.31 9.66-6.58 4.76-11.62 11.33-14.56 20.01-2.94 8.54-4.48 19.73-4.48 33.58 0 13.86 1.54 25.05 4.48 33.59a41.47 41.47 0 0 0 14.56 20.15c6.72 4.9 15.53 8.11 26.31 9.8 10.92 1.67 24.07 2.51 39.75 2.51h-.16zM986.25 219.13c0 14.27-2.3 27.29-6.91 39.04a80.16 80.16 0 0 1-20.3 29.8c-8.95 8.26-19.73 14.56-32.75 19.18-12.87 4.62-27.57 6.85-43.94 6.85h-110.7v65.63h-65.09V248.38H888.8c10.21 0 18.05-2.66 23.5-8.12 5.6-5.32 8.4-12.6 8.4-21.55 0-8.96-2.8-16.1-8.4-21.27-5.45-5.18-13.43-7.84-23.5-7.84H706.4l43.32-66.74h132.81c16.48 0 31.13 2.37 43.92 7.13 12.98 4.76 23.8 11.34 32.61 19.73 8.96 8.4 15.65 18.61 20.3 30.37 4.61 11.75 7 24.77 7 39.04h-.12zM1302.13 313.58l-45.53 66.05h-220.38V122.86h265.28l-45.9 66.74h-148.16v30.79h181.34l-40.73 58.35H1107.4v34.98h194.82zM1579.76 383.97c-4.5 0-8.89-.84-13.15-2.52-4.39-1.68-9.14-5.46-14.5-11.33l-137.02-141.34v150.85h-62.98V166.51c0-8.4 1.08-15.67 3.15-21.82 2.1-6.16 4.9-11.2 8.54-14.98 3.5-3.78 7.41-6.71 12.03-8.4a38.55 38.55 0 0 1 14.28-2.65c4.34 0 8.53.84 12.87 2.52 4.48 1.68 9.24 5.45 14.84 11.33l137.02 141.33V122.86h63.26v212.7c0 8.39-1.12 15.67-3.08 21.82-2.1 6.16-4.9 11.2-8.4 15.12a30.99 30.99 0 0 1-12.18 8.67 41.17 41.17 0 0 1-14.41 2.8h-.27z"/>
            </g>
            <path fill="#C82932" d="M1756.22.21h-67.27s29.26 215.64 29.33 324.11c.04 109.61-29.33 327.54-29.33 327.54h67.27s32.85-217.82 32.76-327.54C1788.94 215.75 1756.22.21 1756.22.21zM1864.9.21h-67.27s29.4 215.64 29.4 324.11c.14 109.61-29.26 327.54-29.26 327.54h67.34s32.9-217.82 32.76-327.54c0-108.57-32.76-324.11-32.76-324.11h-.2z"/><path fill="#000" d="M1960.65 149.78h-42.92s18.62 124.91 18.62 187.76c0 63.52-18.76 189.78-18.76 189.78h42.84s20.86-126.2 20.86-189.86c-.14-62.96-21-187.77-21-187.77l.36.09z"/>
            <g>
              <path fill="#C82932" d="M225.78.21h67.27s-29.26 215.64-29.33 324.11c-.04 109.61 29.33 327.54 29.33 327.54h-67.27s-32.85-217.82-32.76-327.54C193.06 215.75 225.78.21 225.78.21zM117.1.21h67.27s-29.4 215.64-29.4 324.11c-.14 109.61 29.26 327.54 29.26 327.54h-67.2s-32.9-217.82-32.76-327.54C84.27 215.75 117.03.21 117.03.21h.07z"/>
              <path fill="#000" d="M21.35 149.78h42.92S45.65 274.69 45.65 337.54c0 63.52 18.76 189.78 18.76 189.78H21.43S.57 401.12.57 337.46c0-62.96 20.86-187.77 20.86-187.77l-.08.09z"/>
            </g>
          </g>
        </svg>
        Open Powerlifting
      </a>
    </nav>
    <footer>
      <button id="cancel-btn">Ok</button>
    </footer>
  </dialog>
`

const style = `
  dialog {
    font-family: var(--type-body);
    padding: 0;
    box-shadow: 3px 3px rgba(0,0,0,0.1);
    border: 0 none;
    min-width: 280px;
    max-width: 560px;
    left: 50%;
    right: auto;
    transform: translate(-50%, 0);
    transition: opacity 1s ease-in-out;
  }

  dialog[open]::backdrop {
    animation: backdrop-fade 1s ease forwards;
  }

  @keyframes backdrop-fade {
    from {
      background: transparent;
    }
    to {
      background: rgba(0,0,0,0.1);
    }
  }

  dialog header {
    background: none;
    border-right: 0;
    padding: 0.5em;
  }

  dialog nav {
    display: flex;
    flex-direction: column;
    padding: 0.5em;
  }

  dialog nav#social-links a.button {
    border: 0 none;
    justify-content: space-between;
  }

  dialog nav .button:hover {
    background-color: rgba(var(--accent-color), var(--opacity-hover));
  }

  dialog nav .button:active {
    background-color: rgba(var(--accent-color), var(--opacity-active));
  }

  dialog footer {
    text-align: right;
    padding: 0.5em;
  }
`

export function links_dialog(spec) {
  let { _root } = spec;
  const _state = state(spec);
  const _web_component = web_component(spec);

  const init = () => {}

  const effects = () => {
    const dialog = _root.shadowRoot.querySelector('dialog');
    const cancel_button = _root.shadowRoot.querySelector('#cancel-btn');

    dialog.addEventListener('click', (event) => {
      if (event.target === dialog) {
        event.preventDefault();
        close();
      }
    });

    cancel_button.addEventListener('click', (event) => {
      close();
    });
  }

  const show = () => {
    _root.shadowRoot.querySelector('dialog').showModal();
  }

  const close = () => {
    _root.shadowRoot.querySelector('dialog').close();
  }

  return Object.freeze({
    ..._web_component,
    effects,
    show,
    close
  })
}

define_component({
  name: 'wd-links-dialog',
  component: links_dialog,
  template,
  style
});
